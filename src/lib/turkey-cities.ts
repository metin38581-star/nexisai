...
export type TurkishCitySlug = (typeof TURKEY_CITY_OPTIONS)[number]["value"];

const cityLabelMap = new Map(
  TURKEY_CITY_OPTIONS.map((city) => [city.value, city.label]),
);

export function getCityLabel(city: TurkishCitySlug): string {
  return cityLabelMap.get(city) ?? city;
}

export const DEFAULT_CITY: TurkishCitySlug = "kayseri";
